...
class OrbitalPlot(object):
    ...
    def __init__(self, data, x_length=1.0, x_sep=0.1, y_length=0.01):
    ...
    def new_line_data(self, energy):
        """ 
        sets x,y color data to make lin2d object

        """
        degens = degenerate(energy)
        x_start = [-(d // 2) * self.x_shift - (d % 2) * self.x_length / 2.0 for d in degens]
        x_data = []
        for i, j in zip(degens, x_start):
            if i != 1: x_data += [j + k * self.x_shift for k in range(int(i))]
        ...
        y_coor = np.asarray([[i, i] for i in energy])
    ...
    def make_line(self, *args):
        ...
        for i, energy in enumerate(args):
            print i, energy
            x, y, line_color = self.new_line_data(energy)
            ...
            if i == 0:
                self.orb_coor.append([(x_end, y_end)
                                      for ((x_start, x_end), (y_start, y_end)) in zip(x, y)])
                # print "AO1"
                # print self.orb_coor
            elif i == 1:
                self.orb_coor.append([(x_start, x_end, y_end)
                                      for ((x_start, x_end), (y_start, y_end)) in zip(x, y)])
                # print "MO"
                # print self.orb_coor[1]
            elif i == 2:
                self.orb_coor.append([(x_start, y_end)
                                      for ((x_start, x_end), (y_start, y_end)) in zip(x, y)])
                # print "AO2"
            ...
            orbitals = [matplotlib.lines.Line2D(x[a], y[a], color=line_color[a]) for a,_ in enumerate(energy)]
            self.graph.append(orbitals)
            for i in orbitals:
                ax.add_line(i)

            # set graph boundaries
            # self.x_max, self.x_min = self.limit_setter(x, self.x_max, self.x_min)
            # self.y_max, self.y_min = self.limit_setter(y, self.y_max, self.y_min)
            x_max, x_min = self.limit_setter(x[:, 0], x_max, x_min)
            y_max, y_min = self.limit_setter(y[:, 0], y_max, y_min)
